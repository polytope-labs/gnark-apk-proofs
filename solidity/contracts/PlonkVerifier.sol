...
contract PlonkVerifier {
    ...
    uint256 private constant G2_SRS_0_X_0_lo =
    ...
    uint256 private constant G2_SRS_0_X_1_lo =
    ...
    uint256 private constant G2_SRS_0_Y_0_lo =
        78698209480990513415779284580404715789311803115477290401294577488850054555649;
    uint256 private constant G2_SRS_0_Y_0_hi = 17144095208600308495026432580569150746;
    uint256 private constant G2_SRS_0_Y_1_lo =
        91929332261301883145239454754739358796115486554644188311284324629555800144318;
    uint256 private constant G2_SRS_0_Y_1_hi = 8010509799087472606393075511737879449;

    uint256 private constant G2_SRS_1_X_0_lo =
        98818261219665556820116393326710283728194613277906692746999383494736488566849;
    uint256 private constant G2_SRS_1_X_0_hi = 6702007575715075320260787041282023974;
    uint256 private constant G2_SRS_1_X_1_lo =
        107295074956447986529499398282537175562686944614009760601532638792775151105441;
    uint256 private constant G2_SRS_1_X_1_hi = 23095680094956914720643271362915338617;
    uint256 private constant G2_SRS_1_Y_0_lo =
        8594986162647929224598689359123507863380823404425048524967989070164894254109;
    uint256 private constant G2_SRS_1_Y_0_hi = 5726332817694702460752566844701475736;
    uint256 private constant G2_SRS_1_Y_1_lo =
        91300969265257680946581081952456532887094893522716738863049529967274783452774;
    uint256 private constant G2_SRS_1_Y_1_hi = 33997739001153084604046670023305052993;

    uint256 private constant G1_SRS_X_lo =
    ...
    uint256 private constant G1_SRS_Y_lo =
    ...
    // ----------------------- vk ---------------------
    uint256 private constant VK_NB_PUBLIC_INPUTS = 30;
    uint256 private constant VK_DOMAIN_SIZE = 8388608;
    uint256 private constant VK_INV_DOMAIN_SIZE =
        52435868924282763879207214374679607192017058436845975677032209586610279285249;
    uint256 private constant VK_OMEGA = 52167942466760591552294394977846462646742207006759917080697723404762651336366;
    uint256 private constant VK_QL_COM_X_lo =
        49094768722614247555526701986407940029683368408796787060782075245034572348578;
    uint256 private constant VK_QL_COM_X_hi = 6288577142422587810591536689864695630;
    uint256 private constant VK_QL_COM_Y_lo =
        66811764378600404002670590847148369749776287050398399736569458507837199943868;
    uint256 private constant VK_QL_COM_Y_hi = 26147804387696688042099997833083931579;
    uint256 private constant VK_QR_COM_X_lo =
        96587511732286898824560035905922267952767853533527656823238697562582425218494;
    uint256 private constant VK_QR_COM_X_hi = 7553195058771940740009973579094400323;
    uint256 private constant VK_QR_COM_Y_lo =
        81245193510042774406112867084444742978797141379289597080741921827117249299773;
    uint256 private constant VK_QR_COM_Y_hi = 32054548959662733457701125490534001065;
    uint256 private constant VK_QM_COM_X_lo =
        63586623790289529045527711008315943700186274010725973873607301994574094500894;
    uint256 private constant VK_QM_COM_X_hi = 13733525588221543650036377118288624693;
    uint256 private constant VK_QM_COM_Y_lo =
        61655918907267715930346579218011064622392435395748250836665052798405572826700;
    uint256 private constant VK_QM_COM_Y_hi = 18028510621810744597818052887193653176;
    uint256 private constant VK_QO_COM_X_lo =
        61242760671978866585249172720336435750201501864694174319803197134555259457979;
    uint256 private constant VK_QO_COM_X_hi = 15068546599039235548046668379998902237;
    uint256 private constant VK_QO_COM_Y_lo =
        83663133222905979917939000790244604560459443376628865126252738465448758576417;
    uint256 private constant VK_QO_COM_Y_hi = 3810051894554563902376616744795536159;
    uint256 private constant VK_QK_COM_X_lo =
        53082452809001116315885803067515619702877920351767971507020196968769102334844;
    uint256 private constant VK_QK_COM_X_hi = 22268198640635067810199764680341125125;
    uint256 private constant VK_QK_COM_Y_lo =
        9056121615024758487174865563705576904003824973851430806604758393788518004304;
    uint256 private constant VK_QK_COM_Y_hi = 1593516829668077544655340354868020304;

    uint256 private constant VK_S1_COM_X_lo =
        53656634661478696176101473508061484054499488351459486021733893750838719638587;
    uint256 private constant VK_S1_COM_X_hi = 14979971812572356725992609094458091411;
    uint256 private constant VK_S1_COM_Y_lo =
        9306479125468887243552055957444528692441537293869631107161905842900318732120;
    uint256 private constant VK_S1_COM_Y_hi = 16953557688692685394889186806479256766;

    uint256 private constant VK_S2_COM_X_lo =
        5156401246883929809133005921420871664484822216840755991648292045593762678842;
    uint256 private constant VK_S2_COM_X_hi = 13487152165800791762457665944426761536;
    uint256 private constant VK_S2_COM_Y_lo =
        79688563683511324190253588455819966608870967439087633690136121209193866769043;
    uint256 private constant VK_S2_COM_Y_hi = 25721071367046251920054538217859649040;

    uint256 private constant VK_S3_COM_X_lo =
        8745643936880688090254631813338725712188196615049845460217614315634810966338;
    uint256 private constant VK_S3_COM_X_hi = 15699049998988170742200041628079791683;
    uint256 private constant VK_S3_COM_Y_lo =
        24952298611406906097105984704285334588485967860758033911712516865265604060103;
    uint256 private constant VK_S3_COM_Y_hi = 26166212506349745995427735377338759598;

    uint256 private constant VK_COSET_SHIFT = 7;

    uint256 private constant VK_QCP_0_X_lo =
        45046458546911686012183923749250995837292608909915018124833471816680176390408;
    uint256 private constant VK_QCP_0_X_hi = 3427833215558944352780900443123874830;
    uint256 private constant VK_QCP_0_Y_lo =
        45513795090881176615823633566869851733190615923933605632253147195707846774996;
    uint256 private constant VK_QCP_0_Y_hi = 28601738292826775010759210145058130573;

    uint256 private constant VK_INDEX_COMMIT_API_0 = 5162215;
    ...
    // S1(zeta),S2(zeta)
    uint256 private constant PROOF_S1_AT_ZETA = 0x2a0; // Sσ1(zeta)
    uint256 private constant PROOF_S2_AT_ZETA = 0x2c0; // Sσ2(zeta)
    ...
    function Verify(bytes calldata proof, uint256[30] calldata public_inputs) public view returns (bool success) {
        assembly {
            let mem := mload(0x40)
            let freeMem := add(mem, STATE_LAST_MEM)

            // Fixed-size calldata arrays don't expose .offset in Yul.
            // For (bytes calldata, uint256[30] calldata), the ABI layout puts
            // public_inputs inline starting at calldata offset 0x24:
            //   [0x00..0x04) selector
            //   [0x04..0x24) offset pointer to bytes data
            //   [0x24..0x3E4) public_inputs[0..29] (30 × 0x20 bytes inline)
            //   [0x3E4..) bytes length + data

            // sanity checks
            check_inputs_size(VK_NB_PUBLIC_INPUTS, 0x24)
            check_proof_size(proof.length)
            check_proof_openings_size(proof.offset)

            // compute the challenges
            let prev_challenge_non_reduced
            prev_challenge_non_reduced := derive_gamma(proof.offset, VK_NB_PUBLIC_INPUTS, 0x24)
            ...
            let l_pi := sum_pi_wo_api_commit(0x24, VK_NB_PUBLIC_INPUTS, freeMem)
            let l_pi_commit := sum_pi_commit(proof.offset, VK_NB_PUBLIC_INPUTS, freeMem)
            ...
            /// @param p pointer to a marshalled point on BLS12-381(𝔽_p)
            /// @param mPtr free memory
            ...
            function fold_state(aproof) {
                ...
                fr_acc_mul_calldata(
                    add(state, STATE_FOLDED_CLAIMED_VALUES),
                    add(aproof, add(PROOF_QCP_AT_ZETA, 0x0)),
                    acc_gamma
                )
            ...
            function compute_gamma_kzg(aproof) {
                ...
                let check_staticcall :=
                    staticcall(gas(), SHA2, add(mPtr, start_input), 0x3c5, add(state, STATE_GAMMA_KZG), 0x20)
            ...
            function compute_commitment_linearised_polynomial_ec(aproof, s1, coeff_z) {
                ...
                let l_success :=
                    staticcall(gas(), BLS12_MSM_G1, mPtr, 0x6e0, add(state, STATE_LINEARISED_POLYNOMIAL), 0x80)
                if iszero(l_success) {
                    error_ec_op()
                }
            }
