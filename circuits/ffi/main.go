...
import (
	...
	kzgsrs "github.com/consensys/gnark-crypto/kzg"
...
func APKSetup(srsDir *C.char) C.uint64_t {
	...
	dir := srs.DefaultDir()
	if srsDir != nil {
		dir = C.GoString(srsDir)
	}

	var canonicalSRS, lagrangeSRS kzgsrs.SRS
	if dir != "" {
		// Load from disk, downloading from Filecoin ceremony if missing.
		c, l, err := srs.Load(dir, defaultSRSPower)
		if err != nil {
			return 0
		}
		canonicalSRS, lagrangeSRS = c, l
	} else {
		// Unsafe test SRS — not for production.
		c, l, err := unsafekzg.NewSRS(cs)
		if err != nil {
			return 0
		}
		canonicalSRS, lagrangeSRS = c, l
	...
	pk, vk, err := plonk.Setup(cs, canonicalSRS, lagrangeSRS)
	if err != nil {
		return 0
	}
	h := nextHandle.Add(1)
